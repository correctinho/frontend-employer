...
import { CompanyUser } from "../(admin-routes)/dashboard/usuarios/page"
...
export async function fetchCompanyData(business_info_id: string | undefined) {
  ...
  try {
    ...
    if (err.response.data) return { status: err.response.status, data: err.response.data }
...
export const getEmployees = async (q: string, page: number) => {
  const regex = new RegExp(q, "i");
  ...
  try {
    ...
    const employees = response.data
      .filter((employee: any) => regex.test(employee.full_name))
      .slice(ITEM_PER_PAGE * (page - 1), ITEM_PER_PAGE * page)

    return { count: response.data.length, employees }
